...
class TransitionAnimation: UIPercentDrivenInteractiveTransition, UIViewControllerAnimatedTransitioning {
    ...
    override func update(_ percentComplete: CGFloat) {
        ...
        self.fromView?.frame = CGRect(x: kScreenWidth * percentComplete, y: 0, width: kScreenWidth, height: kScreenHeight)
    ...
    func cancel(completedClosure:@ escaping ()->()) {
        ...
        UIView.animate(withDuration: 0.2, animations: {
            self.fromView?.frame = CGRect(x: 0, y: 0, width: kScreenWidth, height: kScreenHeight)
            self.toView.transform = CGAffineTransform(scaleX: default_scale, y: default_scale)
        }) { (completed) in
    ...
    func finish(completedClosure:@ escaping ()->()) {
        ...
        UIView.animate(withDuration: 0.2, animations: {
            self.fromView?.frame = CGRect(x: kScreenWidth, y: 0, width: kScreenWidth, height: kScreenHeight)
    ...
    func pushAnimation(transitionContext:UIViewControllerContextTransitioning) {
        ...
        if transitionContext.viewController(forKey: UITransitionContextViewControllerKey.from) is BaseNavViewController {
            let baseNav = transitionContext.viewController(forKey: UITransitionContextViewControllerKey.from) as? BaseNavViewController
    ...
    func popAnimation(transitionContext:UIViewControllerContextTransitioning) {
        ...
        UIView.animate(withDuration: timeInterval, animations: {
            ...
            fromView.frame = CGRect(x: kScreenWidth, y: 0, width: kScreenWidth, height: kScreenHeight)
        }) { (finished) in
            transitionContext.completeTransition(true)
        }
