...
- (void)requestApiWithHttpRequestModeNoProgeam:(HttpRequestMode *)requestMode
                              Success:(CompletionHandlerSuccessBlock)success
                              Failure:(CompletionHandlerFailureBlock)failure {
    HttpRequest *request = [[HttpRequest alloc]initWithRequestWithName:requestMode.name UrlString:requestMode.url Parameters:nil IsGET:true IsCache:NO];
    [request startRequestWithSuccessBlock:success FailedBlock:failure RequsetStart:nil ResponseEnd:nil];
}
